        ...
        <div class="bulletin">
          ...
          <ul v-if="seller.supports" class="supports">
            <li class="support-item border-1px" v-for="(item,index) in seller.supports">
              <span class="icon-favorite icon" :class="classMap[seller.supports[index].type]"></span>
...
export default {
  ...
  methods: {
    _initScroll() {  //  让右边物品栏实现滚动
      if (!this.scroll) {
        this.scroll = new BScroll(this.$refs.seller, {  //  通过ref
          click: true
        })
      } else {
        this.scroll.refresh()
        ...
        this.$nextTick(() => {
          if (!this.picScroll) {
            this.picScroll = new BScroll(this.$refs.picwrapper, {
              scrollX: true, //  横向滚动
              eventPassthrough: 'vertical'  //  外层可以竖向滚动
            })
          } else {
